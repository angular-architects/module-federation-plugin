...
export function hasRemoteSsr(name: string): boolean {
  ...
  const hasRemoteEntryUrl = Object.values(manifestObject).some(i => i === remoteEntryUrl);
...
const getRemoteEntry = (url: string) => new URL('remoteEntry.json', new URL(url).origin).toString()

export async function getModuleNameByUrl(url: string){

  const remoteEntryUrl = getRemoteEntry(url);
  const result = Object.entries(manifestObject).find(([key, val]) => manifestObject[key] === remoteEntryUrl)
...
export async function loadRemoteModuleSsr<T = any>(options: LoadRemoteModuleOptions) : Promise<T>{

  const {remoteName, exposedModule} = options
  if (!remoteName || !manifestObject[remoteName]) throw new Error('unknown remote ' + remoteName);
  const remoteEntryJson = await getRemoteEntryJson(remoteName)
  if (!remoteEntryJson['exposes'] || !Array.isArray(remoteEntryJson['exposes'])) throw new Error('remoteEntry for "'+remoteName+'" doesn\'t have "exposes"' );

  const exposed = remoteEntryJson['exposes'].find(i => i.key === exposedModule)
  const resultUrl = joinPaths(getDirectory(manifestObject[remoteName]), exposed.outFileName)
...
async function getRemoteEntryJson(remoteName: string): Promise<Record<string, unknown>> {
  if (remoteEntryJsonMap[remoteName]) return remoteEntryJsonMap[remoteName]
  const remoteEntryJson = await fetch(manifestObject[remoteName]).then(r => r.json())
