...
function normalizeShared(config: FederationConfig, skip: Set<string>): Record<string, NormalizedSharedConfig> {
    ...
    result = Object.keys(result).filter(key => !skip.has(key)).reduce((acc, cur) => ({
        ...acc,
        [cur]: result[cur]
    }), {});
...
function normalizeSharedMappings(config: FederationConfig, skip: Set<string>): Array<MappedPath> {
