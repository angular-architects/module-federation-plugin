...
@Component({
  ...
  template: `
    ...
    <p>
      <code>AuthService.userName</code> as seen from mfe1:
      <strong>{{ userName || '(empty)' }}</strong>
    </p>

    <!-- Same component class instance-wise as in the shell: playground-lib is a
         shared singleton, so this renders the name the shell logged in with. -->
